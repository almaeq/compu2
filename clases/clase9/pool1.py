...
def map_():
    ...
    results = pool.map(cube, [3,4,5]) #ejecuta cube() en cada elemento de la lista
    print(results)

    input("Pool.map simple dos args")
    results = pool.starmap(suma, [[3,4],[6,7],[5,8]]) #ejecuta suma() con cada par de elementos de la lista
    print(results)
...
def apply_():
    ...
    results = pool.apply(cube, args=(3,)) #ejecuta cube() con un solo argumento
    print(results)
    ...
    results=[]
    for x in range(15): #ejecuta cube() con cada elemento de la lista
        results.append(pool.apply(cube,args=(x,)))
    print(results)

def mapapplyasync(): #Ejecuta cube en paralelo para cada valor en range(15) de manera asíncrona.
...
def orden(): #emuestra cómo usar pool.apply_async para ejecutar cube_espera en paralelo con un tiempo de espera basado en x
    results=[]
    global cola
