...
if ret > 0:
    print('SOY EL PADRE (PID: %d )' % os.getpid())
    # Cuando bash ejecuta algo como este programa, se queda esperando el exit status 
    #del hijo. Si el padre no espera (os.wait()) no se devolverá el prompt porque bash
    #no busca el exist status del hijo.
    ret = os.wait()
    print(ret)
    print('FIN DEL PADRE')
